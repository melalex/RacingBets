...
CREATE PROCEDURE horse_racing.find_by_racecourse_id(
  ...
  BEGIN
    SET @find_statement = get_races(
        'SELECT * FROM race WHERE race.status = ? AND race.racecourse_id = ? LIMIT ? OFFSET ?');
...
CREATE PROCEDURE horse_racing.find_by_racecourse_name(
  ...
  BEGIN
    SET @find_statement = get_races(
        'SELECT race.id, start_date_time, commission, distance, max_rating, min_age, min_bet, min_rating, race.name, race_class, race_type, status, going, racecourse_id FROM race INNER JOIN racecourse ON race.racecourse_id = racecourse.id WHERE status = ? AND racecourse.name LIKE ? LIMIT ? OFFSET ?');
...
CREATE PROCEDURE horse_racing.find_in_timestamp_diapason(
  ...
  BEGIN
    SET @find_statement = get_races(
        'SELECT * FROM race WHERE status = ? AND race.start_date_time BETWEEN ? AND ? LIMIT ? OFFSET ?');
...
CREATE PROCEDURE horse_racing.find_in_timestamp_diapason_by_racecourse_id(
  ...
  BEGIN
    SET @find_statement = get_races(
        'SELECT * FROM race WHERE status = ? AND race.racecourse_id = ? AND race.start_date_time BETWEEN ? AND ? LIMIT ? OFFSET ?');
